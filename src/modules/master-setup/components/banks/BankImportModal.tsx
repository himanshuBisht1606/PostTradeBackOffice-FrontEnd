...
import { importBankMastersCsv } from '../../services/bankMasterService';
import type { ImportResult } from '../../services/bankMasterService';
...
export function BankImportModal({ open, onClose }: Props) {
  ...
  async function handleImport() {
    ...
    setError(null);
    try {
      const res = await importBankMastersCsv(file);
      setResult(res);
  ...
  return (
    <Modal
      title="Import Bank Master from CSV"
      open={open}
      ...
      {!result ? (
        <>
          ...
          >
            ...
            <p className="ant-upload-hint">Columns (no header): BankCode, BankName, IFSCPrefix</p>
          </Dragger>
        ...
        <Space direction="vertical" style={{ width: '100%' }} size={12}>
          ...
          {result.skipped > 0 && (
            <Alert
              ...
              showIcon
              message={`${result.skipped} row(s) skipped — bank codes already exist.`}
            />
          )}
