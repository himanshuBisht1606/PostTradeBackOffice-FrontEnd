...
import type { CmImportBatchLog } from '../types/cmImport.types';
...
export function CmBatchLogsDrawer({ batchId, batchLabel, onClose }: Props) {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['cm-batch-logs', batchId, page],
    queryFn: () => getCmImportBatchLogs(batchId as string, page, pageSize),
    enabled: !!batchId,
    staleTime: 30_000,
  });

  const columns: TableColumnsType<CmImportBatchLog> = [
    {
      title: 'Row',
      dataIndex: 'rowNumber',
      width: 70,
